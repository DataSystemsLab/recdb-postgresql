...
static int queryAll(AttributeInfo **atts, int numcells, int pheavy, int totalrecs, PGconn *psql) {
	int i, ret, timeint;
...
if (DEBUG) printf("%d heavy cells, %d light cells\n",queryatt->numheavy,queryatt->numlight);
		...
		/* Now we need to adjust the query for the number of recommenders. */
		recstring = (char*) malloc(32*totalrecs*sizeof(char));
		for (j = 0; j < totalrecs; j++) {
			char addition[8];
			if (j == 0) {
				strncpy(recstring,queryatt->recname,strlen(queryatt->recname));
				sprintf(addition," r%d",j);
				strncat(recstring,addition,strlen(addition));
				if (j+1 < totalrecs) strncat(recstring,", ",2);
			} else {
				strncat(recstring,queryatt->recname,strlen(queryatt->recname));
				sprintf(addition," r%d",j);
				strncat(recstring,addition,strlen(addition));
				if (j+1 < totalrecs) strncat(recstring,", ",2);
			}
		...
		qstring = (char*) malloc(1024*sizeof(char));
		sprintf(qstring,"select itemid from %s recommend(10,r0.ratingval) userid=%d",recstring,userid);
...
double execute_workload(char *argrec, int argtotal, int argheavy,
			int argalpha, int argbeta, int arggamma, int argdelta, int totalrecs, PGconn *psql) {
	...
	if (nalpha > 0) {
		int removed = queryAll(alpha, alphacells, nheavy, totalrecs, psql);
		total -= removed;
		talpha -= removed;
	}
	if (nbeta > 0) {
		int removed = queryAll(beta, betacells, 0, totalrecs, psql);
		total -= removed;
		tbeta -= removed;
	}
	if (ngamma > 0) {
		int removed = queryAll(gamma, gammacells, 0, totalrecs, psql);
		total -= removed;
		tgamma -= removed;
	}
	if (ndelta > 0) {
		int removed = queryAll(delta, deltacells, 0, totalrecs, psql);
		total -= removed;
		tdelta -= removed;
	}
...
void issue_queries(char *recommender, int total, int heavy, int alpha, int beta, int gamma, int delta,
			int numrecs, FILE *fp, PGconn *psql) {
	double ttime;
	char *writestring;

	writestring = (char*) malloc(512*sizeof(char));

	ttime = execute_workload(recommender, total, heavy, alpha, beta, gamma, delta, numrecs, psql);
	// Write data.
	sprintf(writestring,"Recommender: %s, using %d recommenders\nAlpha: %d%%, beta: %d%%, gamma: %d%%\nTotal time for %d queries: %f seconds\nAverage query time: %f seconds\n\n",
			recommender, numrecs, alpha, beta, gamma, total, ttime, (ttime/(double)total));
...
void group_of_ten(char *recommender, int total, int heavy, int alpha, int beta, int gamma,
			FILE *fp, PGconn *psql) {
	int i;

	celladjust(recommender, alpha, beta, gamma, 0);
	for (i = 1; i <= 10; i++) {
		issue_queries(recommender, total, heavy, alpha, beta, gamma, 0, i, fp, psql);
	}
}
...
int main(int argc, char *argv[]) {
	...
	if (argc != 4) {
		printf("Usage: ./numrecs recommender total_queries output_file\n");
		exit(0);
	}
	recommender = argv[1];
...
if (DEBUG) printf("%s, %s, %s, %s, %s\n",PQdb(psql), PQuser(psql), PQpass(psql), PQhost(psql), PQport(psql));
	...
	group_of_ten(recommender, total, 80, 100, 0, 0, fp, psql);
	group_of_ten(recommender, total, 80, 0, 100, 0, fp, psql);
	group_of_ten(recommender, total, 80, 0, 0, 100, fp, psql);
