...
ExecSupportsMarkRestore(NodeTag plantype)
{
	switch (plantype)
	{
		...
		case T_RecScan:
			/* It depends on the subscan type. However, we don't have
			 * subscan access here, so let's assume we can't. It might
			 * turn out that we actually can, though, we'll see. */
			return false;
...
ExecSupportsBackwardScan(Plan *node)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_RecScan:
			{
			RecScan* recnode = (RecScan*) node;
			/* We need to look at the subscan. */
			switch (nodeTag(((RecScan*)node)->subscan)) {
				case T_SeqScan:
				case T_TidScan:
				case T_FunctionScan:
				case T_ValuesScan:
				case T_CteScan:
					return TargetListSupportsBackwardScan(recnode->subscan->plan.targetlist);

				case T_IndexScan:
					return IndexSupportsBackwardScan(((IndexScan *) recnode->subscan)->indexid) &&
						TargetListSupportsBackwardScan(recnode->subscan->plan.targetlist);

				case T_IndexOnlyScan:
					return IndexSupportsBackwardScan(((IndexOnlyScan *) recnode->subscan)->indexid) &&
						TargetListSupportsBackwardScan(recnode->subscan->plan.targetlist);

				case T_SubqueryScan:
					return ExecSupportsBackwardScan(((SubqueryScan *) recnode->subscan)->subplan) &&
						TargetListSupportsBackwardScan(recnode->subscan->plan.targetlist);
				default:
					return false;
			}}
