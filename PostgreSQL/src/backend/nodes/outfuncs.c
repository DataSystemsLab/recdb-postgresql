...
_outSelectStmt(StringInfo str, const SelectStmt *node)
{
	...
	WRITE_NODE_FIELD(recommendClause);
...
static void
_outRecommendInfo(StringInfo str, const RecommendInfo *node)
{
...
static void
_outAttributeInfo(StringInfo str, const AttributeInfo *node)
{
	WRITE_NODE_TYPE("ATTRIBUTEINFO");

	WRITE_INT_FIELD(userID);
	...
	else
	{
		...
		switch (nodeTag(obj))
		{
			...
			case T_SelectStmt:
				...
				break;
			case T_RecommendInfo:
				_outRecommendInfo(str, obj);
				break;
			case T_AttributeInfo:
				_outAttributeInfo(str, obj);
				break;
			case T_ColumnDef:
