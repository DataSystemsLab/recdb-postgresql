...
_copySelectStmt(const SelectStmt *from)
{
	...
	COPY_NODE_FIELD(recommendClause);
	COPY_SCALAR_FIELD(op);
...
_copyRecommendInfo(const RecommendInfo *from)
{
...
_copyAttributeInfo(const AttributeInfo *from)
{
	AttributeInfo *newnode = makeNode(AttributeInfo);
...
copyObject(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_RecommendInfo:
			retval = _copyRecommendInfo(from);
			break;
		case T_AttributeInfo:
			retval = _copyAttributeInfo(from);
			break;
