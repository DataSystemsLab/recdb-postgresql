...
create_join_plan(PlannerInfo *root, JoinPath *best_path)
{
	...
	/* Will this be a RecJoin? */
	bool create_recjoin = false;
	...
	/* If one of our two nodes is a RecScan node, then we need to do a RecJoin
	 * instead of a standard join. */
	if (nodeTag(inner_plan) == T_RecScan) {
		/* If it's the inner, we need to swap them. The RecScan is always
		 * the outer node, no exceptions. It's possible that we'll be
		 * joining two RecScans, but that should work alright. */
		plan = inner_plan;
	...
	/* In this case, we might create a RecJoin, which is based on a nested
	 * loop join. */
	...
	/* Now we see what kind of join we create. */
	if (create_recjoin) {
		RecJoin *recjoin;
		Join *subjoin;
		...
		subjoin = (Join *) create_nestloop_plan(root,
											 (NestPath *) best_path,
											 outer_plan,
											 inner_plan);
		recjoin = make_rec_from_join(subjoin);
		plan = (Plan *) recjoin;
	} else {
		/* It's not a RecJoin, so we create a standard join. */
...
/* NEW FOR RECATHON */
/* Really, this is just a straight copy of information. */
Recommend *
make_recommend(Plan *lefttree, RecommendInfo *recInfo)
{
	Recommend  *node = makeNode(Recommend);
	Plan	   *plan = &node->plan;

	/* No point looking at plan cost, this node is
	 * mandatory anyhow. */
	copy_plan_costsize(plan, lefttree);
	...
	node->recommender = recInfo->recommender;
	node->attributes = (void*) recInfo->attributes;
...
is_projection_capable_plan(Plan *plan)
{
	...
	switch (nodeTag(plan))
	{
		...
		/* Since Recathon operators create entirely synthetic data
		 * for a strict set of columns, it's not wise to allow
		 * projection over this data. */
		case T_RecScan:
		case T_RecJoin:
			return false;
		default:
