		...
		case T_RecScan:
			{
				NodeTag type;
				RecScan *recscan;
				/* In case of a RecScan, we need to defer to the subscan. We'll
				 * set its references, store it back in the subscan, and restore
				 * the type tag. Like before, we need to update the subscan plan. */
				...
				recscan->subscan = (Scan *) set_plan_refs(root, (Plan*)recscan->subscan, rtoffset);
		...
		case T_RecJoin:
			{
				...
				set_join_references(root, njoin, rtoffset);

				rjoin->join.plan = njoin->plan;
				rjoin->join.jointype = njoin->jointype;
				rjoin->join.joinqual = njoin->joinqual;
				rjoin->join.plan.type = T_RecJoin;
			}
			break;
