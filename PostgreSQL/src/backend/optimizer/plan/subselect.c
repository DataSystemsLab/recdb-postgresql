...
simplify_EXISTS_query(Query *query)
{
	 ...
	 * are complex. We also can't use a RECOMMEND query for this.
	 */
		...
		case T_RecScan:
			{
				NodeTag type;
				RecScan* recscan;
				Bitmapset* rec_params;

				/* We've had work done to the plan, so we need to ensure the validity
				 * of the subscan plan before we do this. */
				recscan = (RecScan*) plan;
				type = recscan->subscan->plan.type;
				recscan->subscan->plan = recscan->scan.plan;
				recscan->subscan->scanrelid = recscan->scan.scanrelid;
				recscan->subscan->plan.type = type;

				/* We need to perform the parameter adjustment of whatever our
				 * subscan is. Unfortunately this doesn't make the code very
				 * pretty. */
				rec_params = finalize_plan(root, (Plan*)recscan->subscan,
						valid_params, scan_params);
				context.paramids = bms_add_members(context.paramids, rec_params);

				recscan->scan.plan = recscan->subscan->plan;
				recscan->scan.scanrelid = recscan->subscan->scanrelid;
		...
		case T_RecJoin:
			{
				...
				nplan = (Plan*) ((RecJoin*) plan)->subjoin;

				finalize_primnode((Node *) ((Join *) nplan)->joinqual,
								  &context);
				// collect set of params that will be passed to right child
				foreach(l, ((NestLoop *) nplan)->nestParams)
				{
					NestLoopParam *nlp = (NestLoopParam *) lfirst(l);
				...
				/* Now we replace the joinqual. */
				jrec = (Join*) plan;
				jrec->joinqual = ((Join*) nplan)->joinqual;
			}
			break;
