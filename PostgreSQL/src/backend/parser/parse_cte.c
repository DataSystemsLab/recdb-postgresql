...
checkWellFormedRecursion(CteState *cstate)
{
	...
	for (i = 0; i < cstate->numitems; i++)
	{
		...
		if (stmt->recommendClause)
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("RECOMMEND in a recursive query is not implemented"),
					 parser_errposition(cstate->pstate,
							   exprLocation((Node *) stmt->recommendClause))));
	}
}
		...
		switch (stmt->op)
		{
			...
			case SETOP_INTERSECT:
				...
				checkWellFormedRecursionWalker((Node *) stmt->lockingClause,
											   cstate);
				/* NEW FOR RECATHON */
				checkWellFormedRecursionWalker((Node *) stmt->recommendClause,
											   cstate);
