 ...
 *		NEW FOR RECATHON
 *
 * When a user issues a RECOMMEND query, we will end up using our SelectRec operator instead
 * of a standard scan operator, but we still need to have the ability to scan in any way that
 * PostgreSQL normally can (at least for the sake of safety). Thus we have this struct, which
 * stores a scan plan together with the recommend operator.
 * ----------------
 */
typedef struct RecScan
{
	Scan		scan;		/* necessary for compatibility */
	Node		*recommender;	/* this is a RecommendInfo node */
	Scan		*subscan;	/* the actual scan */
} RecScan;
...
typedef struct RecJoin
{
	Join		join;		/* necessary for compatibility */
	Join		*subjoin;	/* the actual join */
} RecJoin;
 ...
 *		recommend node
 *
 * We're basically copying the RecommendInfo structure into a
 * Recommend node.
 * ----------------
 */
