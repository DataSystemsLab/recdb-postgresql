...
/* ----------------
 *	 RecScanState information
 *
 *		RecScan nodes are used for recommend queries and contain another ScanState.
 * ----------------
 */
 ...
 *	NEW FOR RECATHON
 *
 *	 RecommendState information
 *
 *		Recommend nodes are used to enforce RECOMMEND clauses.
 *		They select all tuples, then use the Recathon Java program
 *		to assign a rating value to each tuple.
 *
 * TupleArray is the current array of tuples paired with float ratings.
 * count is the current number of the tuple being returned. Once this reaches
 * recLimit, our work is done, and we return NULL. Additionally, if count is
 * 0, then we need to perform the actual ranking mechanism.
 * ----------------
 */

typedef enum {
	RECOMMEND_SCAN,		// if we're scanning for current tuples
	RECOMMEND_RETURN,	// if we're returning batch-processed tuples
	RECOMMEND_END		// if we're out of tuples to process
} RecommendStateCond;

/* A structure for holding a HeapTuple and its itemID together. */
...
/* Used to store the list of the top-K ranked tuples. */
typedef struct TupleRating
{
	HeapTuple		slot;
	float			rank;
} TupleRating;

typedef struct TupleRecState
{
	MemoryContext	recContext;
	TupleRating	*tupleRank;
	int64		k;
	int64		current;		/* the current tuple to be returned */
	int64		length;			/* the length of the array, so we know to clip the tail */
	float		minRating;		/* the current smallest rating in the table */
} TupleRecState;

typedef struct RecommendState
{
	PlanState		ps;		/* its first field is NodeTag */
	RangeVar		*recommender;	/* the recommender we're referring to */
	char			*ratingTable;	/* the rating table of the recommender */
	ExprState		*recLimit;	/* K parameter */
	Node			*recOptions;	/* the specific recommender options to be sent */
	void			*attributes;	/* attribute information */
	int64			k;		/* the overall k, obtained from recLimit */
	bool			listMade;	/* whether or not we've constructed our top-k list */
	TupleRecState		*recState;	/* memory state for storing slots */
	// SelectRecommend only below.
	int			sockfd;		/* our connection socket */
	int 			batchCount;	/* which entry to return */
	int			tuples;		/* the number of tuples processed */
	long long int		updatetime;	/* the time of the most recent table update */
	struct sockaddr_in	serv_addr;	/* our connection struct */
	tupleItem**		batchList;	/* our stored batch list */
	tupleItem**		hashTable;	/* our stored hash table */
	RecommendStateCond	state;		/* information about our state machine */
} RecommendState;
