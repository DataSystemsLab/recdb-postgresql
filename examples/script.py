...
def main():
	...
	print "Recommendation: Dataset MovieTweet"
	...
	cursor.execute("create table if not exists ratings (userid int, itemid int, rating int);");
	conn.commit()
	...
	executionStart = time.time()	
	query =	"copy ratings(userid, itemid, rating) from "+RatingDataPath+" DELIMITER ':';"
	...
	cursor.execute("create table if not exists users(userid int, other real);");
	conn.commit()	
	
	query = "copy users(userid, other) from  "+UserDataPath+" DELIMITER ':';"
	cursor.execute(query);
	conn.commit()
		
			
	cursor.execute("create table if not exists moives( itemid int, name varchar, genre varchar, other varchar);");
	conn.commit()

	query = "copy moives(itemid, name, genre, other) from "+ItemDataPath+" DELIMITER ':';"
	cursor.execute(query);
	conn.commit()
	...
	executionStart = time.time()	
	cursor.execute("CREATE RECOMMENDER mtitemcos on ratings Users FROM userid Items FROM itemid Events FROM rating Using ItemCosCF;");
	conn.commit()
	executionTime = time.time() - executionStart
	print "\n"
	print (" Execution time is  :-", executionTime) 
	...
	print "\n \n  Single Join Query.."		
	executionStart = time.time()	
	cursor.execute("select r.userid, r.itemid, r.rating, b.other from ratings r, moives a, users b Recommend itemid to userid ON rating using ItemCosCF where r.userid = b.userid and r.userid = 21;");
	conn.commit()
	executionTime = time.time() - executionStart
	print "\n"
	print (" Execution time is  :-", executionTime) 
	...
	print "\n \n Creating Recommender.."	
	executionStart = time.time()	
	cursor.execute("CREATE RECOMMENDER mtitempear on ratings Users FROM userid Items FROM itemid Events FROM rating Using ItemPearCF;");
	conn.commit()
	executionTime = time.time() - executionStart
	print "\n"
	print (" Execution time is  :-", executionTime) 
	...
	print "\n \n  Single Join Query.."		
	executionStart = time.time()	
	cursor.execute("select r.userid, r.itemid, r.rating from ratings r, moives a Recommend itemid to userid ON rating using ItemPearCF where r.itemid = a.itemid and a.itemid = 35423;");
	conn.commit()
	executionTime = time.time() - executionStart
	print "\n"
	print (" Execution time is  :-", executionTime) 
	...
	print "\n \n Creating Recommender.."	
	executionStart = time.time()	
	cursor.execute("CREATE RECOMMENDER mtitemsvd on ratings Users FROM userid Items FROM itemid Events FROM rating Using SVD;");
	conn.commit()
	executionTime = time.time() - executionStart
	print "\n"
	print (" Execution time is  :-", executionTime) 
	...
	print " \n \n Single Join Query.."		
	executionStart = time.time()	
	cursor.execute("select r.userid, r.itemid, r.rating from ratings r, moives a Recommend itemid to userid ON rating using SVD where r.itemid = a.itemid and a.itemid = 35423;");
	conn.commit()
	executionTime = time.time() - executionStart
	print "\n"
	print (" Execution time is  :-", executionTime) 
	...
	cursor.execute("drop table moives;");
	conn.commit()
	...
	print "Recommendation: Dataset MovieLens 100k"	
	...
	################################################################
	################################################################
	print "Recommendation: Dataset MovieLens 1M"	
